...
% Require to run SURF_Homography first to generate: H_SURF{n}
...
r=0.001;    %std of measurement
Qi = q^2*eye(3); 
Q  = blkdiag(zeros(3,3),Qi,zeros(3,3),Qi,Qi); % covariance of process
R  = r^2*eye(3);%blkdiag(q^2*eye(3),r^2*eye(3));        % covariance of measurement  
...
for n = 1:imgNum-1
    ...
    [R1,T1,t1,angle1,lambda1,R2,T2,t2,angle2,lambda2] = decompHomoMatrix2(H_SURFRea);
    ...
    P_t(:,1) = t1; 
    P_t(:,2) = t2; 
    P_test = abs([t1(3),t2(3)]);
    node = find(P_test==min(P_test));
    z_t = P_t(:,node);%[angle_t{node};P_t(:,node)];
    for i = imgStamp(1):imgStamp(n+1)
